...
if Datasource!=3:
 ...
 # y_theor-=mean(y_theor) 
  ...
  Hmean_theor_refl=[]
  xwall = loadtxt("xw.txt")
  for xp in probe_x:
   Hmean_theor_refl.append(2.0*abs(cos(k*(xp-xwall))))
  ...
  plot(probe_x,Hmean,'ko',lw=2.5,label=r'$L_{ga}/\lambda$=0.5') 
  plot(probe_x,Hmean_theor_refl,'k-',lw=0.8, label='Linear theory')
  i1 = where(probe_x > xwall-10.)[0][0]  
  RMS =  (Hmean[i1:] - Hmean_theor_refl[i1:])* (Hmean[i1:] - Hmean_theor_refl[i1:])
  RMS = mean(RMS)
  RMS = sqrt(RMS)
  print RMS
  legend(bbox_to_anchor=[0.95,0.99],ncol=2,fontsize=14)
  xlabel(r'$x$ (m)', fontsize=16)
  ylabel(r'$K_m $(%)',fontsize=16)  
  xlim(int(GenerationZoneLength)-0.1,xwall+0.1)
  ylim(0,3.5)
  savefig('meanH.png',dpi=150)
